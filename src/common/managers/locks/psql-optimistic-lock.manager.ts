...
@Injectable()
export class OpLockManager implements ILockManager{
    constructor(    
    ...
    async withLockBySrc<T>(resourceId: string, resourceType: string, operation: (transactionalEntityManager) => Promise<T>): Promise<T> {
        ...
        try {
            ...
            // concertId에 대해 optimistic 락을 사용하여 동시성 제어
            const entity = await lockRepository.findOne({ where: { id: resourceId } });
            const version = entity?.updated_at || new Date(); // Add now if version is undefined
            const lock = await lockRepository.findOne({
                where: { id: resourceId },
                lock: { mode: 'optimistic', version: version },
            });
